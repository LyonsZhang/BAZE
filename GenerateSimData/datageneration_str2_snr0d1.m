...
sigma=mean([abs(b1),abs(b2)])/0.1; % noise variance.
...
datafile1 = fullfile([type '_snr' num2str(round(snr,1)) 'simdata.xlsx']);
datafile2 = fullfile([type '_snr' num2str(round(snr,1)) 'simdata.mat']);

xlswrite(datafile1,Y,1)
xlswrite(datafile1,X,2)
writetable(coeffs,datafile1,'Sheet',3)
save str2_snr0d1.mat
