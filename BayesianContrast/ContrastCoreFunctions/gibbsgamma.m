function [gamma,betahat,MSE,nselect,Yhat,etahat]=gibbsgamma(nburnin,niter,p,nop,Y, X,T,Xs,Ts, a, Q, n,tau,nu,omega,seed,predict,stand,display)  
...
%    rng(seed);
    ...
    if isempty(stand)
        ...
        %Xobs=Xs*diag(stand.Sx)+stand.mux;
    end
    ...
    if predict
        %initialize beta
        etahat=zeros(p,1);
        betahat=zeros(p+1,nburnin+niter+1);
        tembeta=invAri*Xri'*Y;
        etahat(index)=Sy*invSx(index,index)*tembeta;
        etahatold=etahat;
        betahat(:,1)=Ts*etahat;
...
%            invAri=Ari\eye(length(indxtemp));
            ...
            elseif (newgamma~=0) || (tn==1)
                ...
                if predict
                    etahat=etahatold;
                    betahat(:,i+1)=betahatold;
                    Yhat(:,i+1)=Yhatold;
                    MSE(i+1)=MSEold;
%                     betahat(:,i+1)=Ts*etahat;
%                     Yhat(:,i+1)=(Xobs)*betahat(:,i+1);
