...
load str1_snr10_n100_p1000.mat
...
for repeat=1:rn
    if doroc
    [MSEpre,MSEtest,average,sem,FPR,FNR,FPN,FNN,sensitivity,specificity,precision,accuracy,etafinal,roc]=traintest(k,pid,nburnin,niter,p,Y, X, T, Xs, Ts,beta, a, Q, n,tau,nu,omega,seed,predict,cutoff,stand,pl,true_index,pathname,figname,doroc);
...
%     raccuracy(((repeat-1)*k+1):(repeat*k),:)=roc.raccuracy;
    rAUC(repeat,:)=roc.rAUC; 
        ...
        [MSEpre,MSEtest,average,sem,FPR,FNR,FPN,FNN,sensitivity,specificity,precision,accuracy,etafinal]=traintest(k,pid,nburnin,niter,p,Y, X, T,Xs, Ts, beta, a, Q, n,tau,nu,omega,seed,predict,cutoff,stand, pl,true_index,pathname,figname,doroc);
    end
...
if doroc
    ...
    xlswrite(fullfile(pathname,'roc.xlsx'),AUCm,'Sheet3');
