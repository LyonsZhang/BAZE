...
x=[350:10:450];                  %#initialize x array
y1=0.6+zeros(1,length(x));                      %#create first curve
y2=0.8+zeros(1,length(x));                   %#create second curve
X6=[x,fliplr(x)];                %#create continuous x value array for plotting
Y6=[y1,fliplr(y2)];  %#create y values for out and then back
